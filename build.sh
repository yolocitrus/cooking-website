...
ORI_NAME_ARRAY=()
SLU_NAME_ARRAY=()
for i in `ls cooklang_recipes | grep .cook`; do
    ORI_NAME_ARRAY+=(cooklang_recipes/$i)
    SLU_NAME_ARRAY+=(cooklang_recipes/$(pipenv run slugify `echo $i | sed 's/.cook//g'`).cook)
done
...
for CATEGORY in $(echo "$CATS" | cut -d" " -f2- | sed 's/^[ \t]*//;s/[ \t]*$//' | sort | uniq); do
    ...
    #CATEGORY_FAUX_URLENCODED=`pipenv run slugify $CATEGORY`
...
for FILE in _recipes/*.md; do
    ...
    if [[ "$GITHUB_ACTIONS" = true ]]; then
        SLU_COOK_FILE=`echo $FILE | sed 's/_recipes/cooklang_recipes/g;s/.md/.cook/g'`
        echo $SLU_COOK_FILE
        for i in "${!SLU_NAME_ARRAY[@]}"; do
            if [ "${SLU_NAME_ARRAY[$i]}" = "$SLU_COOK_FILE" ]; then
                ORI_COOK_FILE=${ORI_NAME_ARRAY[$i]}
                break
            fi
        done
        UPDATED_AT="$(git log -1 --date=short-local --pretty='format:%cd' "$ORI_COOK_FILE")"
    else
